...
start_time = time.time()
target_layers = [model.features[7]]#[model.layer4[-1]] #na dokimaso kai me .conv3 sto ka8ena
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.features[4]]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
input_tensor = inputs
model = model1
target_layers = [model.features[7]]#features[7]
