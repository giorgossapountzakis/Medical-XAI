...
path='/data/data1/users/el17074/mymodels/densenet/densenet121gridsearched/'
fcmiddlenumber=256#model0_params['fcmiddlenumber']#500
model1= models.densenet121()
num_ftrs = model1.classifier.in_features #classifier[0] gia vgg, classifier gia densenet,fc giaresnet
...
start_time = time.time()
target_layers = [model.features.denseblock3.denselayer24]#[model.layer4[-1]] #na dokimaso kai me .conv3 sto ka8ena
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.features.denseblock2.denselayer12]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.features.denseblock1.denselayer6]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
