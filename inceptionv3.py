...
norm_transforms = transforms.Compose([transforms.Resize(299),#299gia inception
...
if numberoftraininglayers==3:
    ...
    for param in model1.Mixed_7c.parameters():
        param.requires_grad = True
    for param in model1.Mixed_6a.parameters():
        param.requires_grad = True
    ...
    for param in model1.Mixed_6e.parameters():
        param.requires_grad = True
    for param in model1.Mixed_5b.parameters():
        param.requires_grad = True
    for param in model1.Mixed_5c.parameters():
        param.requires_grad = True
    for param in model1.Mixed_5d.parameters():
        param.requires_grad = True
...
model1, hist,best_acc = train_model(model1, dataloaders_dict, criterion, optimizer,scheduler, num_epochs=8, is_inception=True)
...
def grid_search(dataloaders_dict, num_epochs):
    ...
    for lr, momentum, step_size, gamma ,numberoftraininglayers,fcmiddlenumber in param_combinations:
        ...
        if numberoftraininglayers==2 or numberoftraininglayers==3:
          model_temp, hist ,train_best_acc= train_model(model1, dataloaders_dict, criterion, optimizer, scheduler, num_epochs, is_inception=True)
        else:
          model_temp, hist ,train_best_acc= train_model(model1, dataloaders_dict, criterion, optimizer, scheduler, num_epochs, is_inception=True)
