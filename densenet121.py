...
image_datasets = {x: datasets.ImageFolder('/data/data1/users/el17074/my_data/Lung Segmentation Data/'+x, transform=my_transforms) for x in ['Masked_Train','Test', 'Masked_Val','Small_Test']}
...
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#"cuda:1" 
...
path='/data/data1/users/el17074/mymodels/densenet/densenet121remasked/'
print(path)
...
'''
path='/data/data1/users/el17074/mymodels/densenet/densenet121regridsearched/'
print(path)
def grid_search(dataloaders_dict, num_epochs):
    ...
    for lr, momentum, step_size, gamma ,numberoftraininglayers,fcmiddlenumber in param_combinations:
        ...
        if numberoftraininglayers==2:
          model_temp, hist ,train_best_acc= train_model(model, dataloaders_dict, criterion, optimizer, scheduler, num_epochs+3, is_inception=False)
        else:
...
model1= models.densenet121(weights=DenseNet121_Weights.DEFAULT)
...
#f.write('densenet121')
#f.write('0.01,2,256')
#f.write('numberoftraininglayers= 1')
#f.write('fcmiddlenumber= 500')
