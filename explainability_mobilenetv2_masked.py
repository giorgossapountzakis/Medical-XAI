...
train_data_path='/data/data1/users/el17074/my_full_data/Lung Segmentation Data/Masked_Train'
valid_data_path='/data/data1/users/el17074/my_full_data/Lung Segmentation Data/Masked_Val'
test_data_path='/data/data1/users/el17074/my_full_data/Lung Segmentation Data/Masked_Test'
small_test_data_path='/data/data1/users/el17074/my_full_data/Lung Segmentation Data/Masked_Small_Test'

"""# normalization"""

norm_transforms = transforms.Compose([transforms.Resize(256),
                                      transforms.CenterCrop(225), #isos na to allakso edo an bgazei skata efficiency
                                      transforms.ToTensor(),
...
start_time = time.time()
target_layers = [model.features[14]]#[model.layer4[-1]] #na dokimaso kai me .conv3 sto ka8ena
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.features[6]]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
