...
norm_transforms = transforms.Compose([transforms.Resize(256),#299gia inception
                                      transforms.CenterCrop(224),#299gia inception
                                      transforms.ToTensor(),
...
image_datasets = {x: datasets.ImageFolder('/data/data1/users/el17074/my_full_data/Lung Segmentation Data/'+x, transform=my_transforms) for x in ['Train','Test', 'Val']}
...
if numberoftraininglayers==3:
    for param in model1.features[4:].parameters():
...
model1.load_state_dict(torch.load('/data/data1/users/el17074/mymodels/efficientnet/efficientnetgridsearched/'+'weights.pt'))
...
optimizer= optim.SGD(model1.parameters(), lr=0.05, momentum=0.9)
criterion = nn.CrossEntropyLoss()
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.1)
# Train and evaluate
start_time = time.time()
model1, hist,best_acc = train_model(model1, dataloaders_dict, criterion, optimizer,scheduler, num_epochs=15, is_inception=False)
...
path='/data/data1/users/el17074/mymodels/efficientnet/efficientnethyperparameters/'
def grid_search(dataloaders_dict, num_epochs):
    # Define the parameter grid
    param_grid = {'lr': [0.05,0.01,0.005],
    ...
    for lr, momentum, step_size, gamma ,numberoftraininglayers,fcmiddlenumber in param_combinations:
        ...
        num_ftrs = model1.classifier[1].in_features
        model1.classifier =  nn.Sequential(OrderedDict([('fc1', nn.Linear(num_ftrs, fcmiddlenumber)),('relu', nn.ReLU()),('dropout',nn.Dropout()),('fc2', nn.Linear(fcmiddlenumber, 3))]))
        model1.load_state_dict(torch.load('/data/data1/users/el17074/mymodels/efficientnet/efficientnetmasked/'+'weights.pt'))
        model1 = model1.to(device)
