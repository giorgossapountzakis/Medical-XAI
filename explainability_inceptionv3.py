...
start_time = time.time()
target_layers = [model.Mixed_6e.branch_pool]#[model.layer4[-1]] #na dokimaso kai me .conv3 sto ka8ena
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.Mixed_5d.branch_pool]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
target_layers = [model.Conv2d_4a_3x3]#[model.layer4[-1]]
with GradCAM(model=model, target_layers=target_layers,use_cuda=True) as cam:
  im = transforms.ToPILImage()(inputs[covposition]).convert('RGB')
  im = np.asarray(im, dtype="float32" )/255
  input_tensor = inputs
...
def benchmark2(input_tensor, target_layers, eigen_smooth=False, aug_smooth=False, category=0):
    methods = [("FullGrad", RandomCAM(model=model, target_layers=target_layers, use_cuda=True)),
...
input_tensor = inputs
model = model1
target_layers = [model.Mixed_6e.branch_pool]
