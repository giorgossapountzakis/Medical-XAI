...
image_datasets = {x: datasets.ImageFolder('/data/data1/users/el17074/my_data/Lung Segmentation Data/'+x, transform=my_transforms) for x in ['Masked_Train','Test', 'Masked_Val']}
...
path='/data/data1/users/el17074/mymodels/squeezenet/squeezenetmasked/'
...
path='/data/data1/users/el17074/mymodels/squeezenet/squeezenetgridsearched/'
def grid_search(dataloaders_dict, num_epochs):
    # Define the parameter grid
    param_grid = {'lr': [0.001,0.01,0.05],
                  'momentum': [0.9],
                  'step_size': [5,8],#isos bgalo to 11
                  'gamma': [0.1],
    ...
    for lr, momentum, step_size, gamma ,numberoftraininglayers,fcmiddlenumber in param_combinations:
        ...
        if numberoftraininglayers==1:
            for param in model1.features[9:].parameters():
                param.requires_grad = True
        ...
        if numberoftraininglayers==2 or numberoftraininglayers==3:
          model_temp, hist ,train_best_acc= train_model(model1, dataloaders_dict, criterion, optimizer, scheduler, num_epochs+5, is_inception=False)
        else:
...
start_time = time.time()
model0,model0_params,model0_acc,model0_hist=grid_search(dataloaders_dict, 7)
