...
norm_transforms = transforms.Compose([transforms.Resize(254),#256
                                      transforms.CenterCrop(226),#224
                                      transforms.ToTensor(),
...
targets = [ClassifierOutputSoftmaxTarget(0)]#analoga tin katigoria poy einai h eikona
...
print(f"CAM, Combined metric avg confidence increase with ROAD accross 3 thresholds (positive is better): {scores[0]}")
k=open(path+'total ROAD.txt','w')
k.write(f"CAM, Combined metric avg confidence increase with ROAD accross 3 thresholds (positive is better): {scores[0]}")
k.close()
...
e = shap.GradientExplainer((model1, model.layer4[-1]), inputs)
