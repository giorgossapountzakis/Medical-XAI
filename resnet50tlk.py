...
image_datasets = {x: datasets.ImageFolder('/data/data1/users/el17074/my_full_data/Lung Segmentation Data/'+x, transform=my_transforms) for x in ['Train','Test', 'Val','Small_Test']}
...
model1.load_state_dict(torch.load('/data/data1/users/el17074/mymodels/resnet/resnet50gridsearched/'+'weights.pt'))
model1 = model1.to(device)


"""# training(comment all if gridsearch)"""

path='/data/data1/users/el17074/mymodels/resnet/resnet50remasked/'
print(path)
...
path='/data/data1/users/el17074/mymodels/resnet/resnet50hyperparameters2/'

def grid_search(dataloaders_dict, num_epochs):
    # Define the parameter grid
    param_grid = {'lr': [0.05,0.01],
                  'momentum': [0.9],
                  'step_size': [3,5],
                  'gamma': [0.1],
                  'numberoftraininglayers':[1,2,3],
                  'fcmiddlenumber': [512]
                  }
	...
	#model training depth
        ...
        count=count+1
        print(count)
        model.fc =  nn.Sequential(OrderedDict([('fc1', nn.Linear(num_ftrs, fcmiddlenumber)),('relu', nn.ReLU()),('dropout',nn.Dropout()),('fc2', nn.Linear(fcmiddlenumber, 3))]))
        model.load_state_dict(torch.load('/data/data1/users/el17074/mymodels/resnet/resnet50gridsearched/'+'weights.pt'))
...
#f.write('resnet50')
#f.write('epochs = 7')
#f.write('numberoftraininglayers= 3')
#f.write('fcmiddlenumber= 512')

#f.write(str(model0_acc)+'\n')
#f.write(str(model0_params['lr'])+'\n')
#f.write(str(model0_params['momentum'])+'\n')
#f.write(str(model0_params['step_size'])+'\n')
#f.write(str(model0_params['gamma'])+'\n')
...
model = model1
target_layers = [model.layer4[-1]]#[model.layer4[-1]]
...
for i in range(0,3):
  ...
  plt.savefig('/data/data1/users/el17074/mymodels/resnet/resnet50/lime1.png')
